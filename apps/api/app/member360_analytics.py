...
def members_for(user, roles):
    ...
    for m in visible_profiles(user, roles):
        ...
        output.append(dict(id=m['id'], name=m['name'], age=m['age'], sex=m['sex'],
            ...
            analytics_contract=m['contract'], profile_reference=True, profile_rank=_DATA['members'].index(m),
...
def fixtures(members):
    ...
    for m in _DATA['members']:
        ...
        for i, row in enumerate(rows):
            ...
            cat = 'OC' if 'Unsupported' in category else 'RC' if 'Recapture' in category else 'SP' if 'Specificity' in category else 'CG' if category == 'Validated' else 'NC'
            kind = {'OC': 'conflicting', 'RC': 'recapture', 'SP': 'specificity', 'CG': 'invisible', 'NC': 'disconnected'}[cat]
            key = f'M360-{m["id"]}-{i+1}'
            findings.append(dict(id=key, member_id=m['id'], condition=name, business_category=cat,
                ...
                evidence='Strong' if category == 'Validated' else 'Moderate' if cat in ('RC', 'SP') else 'Limited',
                ...
                summary=evidence, countercheck=check + ' Confirm current-period documentation and model mapping before any coding change.',
                profile_reference=dict(member_id=m['id'], year=2026, category=category, confidence=confidence,
                ...
                discovery=dict(kind=kind, label=labels[kind], signal=evidence,
                    ...
                    comparison_basis='Member 360 reference summary; original clinical documents are not attached.', version='member360-linked-v1')))
    return findings
