...
Practice IDs and clinical/source records remain unchanged. Explicit Member 360
hierarchy values are preserved; the wider population uses the supplied networks.
"""
from hashlib import sha256
from .people import ACCOUNT_NAMES

NETWORKS = ['Central MA Network', 'Northside Medical', 'Harbor Primary Care', 'Dr. A. Carter']
...
def identity(member):
    ...
    return dict(practiceId=pid, network=member.get('health_network') or NETWORKS[(index // 2) % len(NETWORKS)],
