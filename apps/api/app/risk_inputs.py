...
def member_input(member, config_id=DEFAULT_CONFIG):
    ...
    age = int(member.get('age', 70))
    if hhs:
        age = (42, 17, 1, 21, 20, 2, 0)[(n-1) % 7]
    age_year = 2026 if hhs else payment_year
    age = max(1, min(age, 98)) if not hhs else age
    # The synthetic Medicare DOB is stable across payment years. Age changes
    # at the official reference date; a period selector never changes identity.
    birth_year = (2026 if hhs else 2027) - age
    values = {'member_id': member['id'], 'dob': f'{birth_year}-01-15',
              'sex': 1 if member.get('sex') == 'Male' else 2, 'orec': 1 if age < 65 and not hhs else 0,
              'enrollment': [], 'diagnoses': [], 'ndc': [], 'hcpcs': [], 'synthetic': True,
              'fixture_version': FIXTURE_VERSION, 'program': 'ACA' if hhs else 'Part D' if config_id.startswith('rxhcc') else 'MA',
...
def manifest(members, config_id=DEFAULT_CONFIG):
    ...
    return {'version': FIXTURE_VERSION, 'synthetic': True, 'expected_members': len(members),
