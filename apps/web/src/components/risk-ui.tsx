...
import type { RiskPrecision, RiskConfiguration, RiskRun, RiskMemberProfile, RiskScenarioResult, ScoreBasis } from "@/lib/risk-types";
...
export function RiskOverview({ user, embedded = false }: { user: User; embedded?: boolean }) {
  ...
  return <div className="risk-workspace">
    ...
    {context.error ? <RiskError error={context.error} /> : result.isPending ? <LoadingRisk /> : result.error ? <RiskError error={result.error} retry={() => result.refetch()} /> : overview ? <>
      ...
      {!!overview.external_groups?.length && <Panel title="Comparable external score groups" subtitle="Producer, program, rating period and normalization basis remain explicit."><div className="table-scroll"><table className="risk-table"><thead><tr><th>Producer / model</th><th>Program / period</th><th>Normalization basis</th><th>Coverage</th><th>Raw / adjusted score</th></tr></thead><tbody>{overview.external_groups.map((group, index) => <tr key={index}><td><strong>{group.producer}</strong><small>{group.model_version}</small></td><td>{group.program}<small>{group.rating_period} · {group.rate_cells.join(", ")}</small></td><td>{group.normalization_basis}</td><td>{group.members} members<small>{group.member_months} member-months</small></td><td>{formatRiskScore(group.raw_score)}<small>{formatRiskScore(group.adjusted_score)} adjusted</small></td></tr>)}</tbody></table></div></Panel>}
      <div className="risk-overview-grid">
        <Panel title="Risk distribution" subtitle={`${scoreBasisLabels[context.basis]} · ${overview.portfolio.weighting || "Selected population"}`}>
          {overview.distribution.length ? <div className="risk-distribution">{overview.distribution.map((bucket) => <div key={bucket.label}><span>{bucket.label}</span><div><i style={{ width: `${Math.max(0, bucket.count) / Math.max(1, ...overview.distribution.map((b) => b.count)) * 100}%` }} /></div><strong>{num(bucket.count)}</strong></div>)}</div> : <Empty title="No completed distribution" description="Calculate the eligible population to inspect its distribution." />}
          <div className="risk-panel-foot"><span>{num(overview.coverage.scored_member_months)} scored member-months</span><Link href={coverageHref}>Inspect coverage<ArrowRight size={13} /></Link></div>
        </Panel>
        <Panel title="Calculation coverage" subtitle="Excluded and failed records remain part of the reconciliation.">
      ...
      <Panel title="Member risk results" subtitle="Each result opens its retained input and factor ledger." action={<Link href={context.href("/members")}>All members<ArrowRight size={14} /></Link>}>
        {overview.members.length ? <div className="table-scroll"><table className="risk-table"><thead><tr><th>Member</th><th>Raw score</th><th>Adjusted score</th><th>Calculation state</th><th>Result</th></tr></thead><tbody>{overview.members.map((member) => <tr key={member.member_id}><td><strong>{member.name || member.member_id}</strong><small>{member.member_id}</small></td><td className="risk-number">{formatRiskScore(member.raw_score, context.configuration?.precision)}</td><td className="risk-number">{formatRiskScore(member.adjusted_score, context.configuration?.precision)}</td><td>{label(member.stale ? "stale" : member.status || (member.run_id ? "completed" : "not_calculated"))}</td><td><Link href={context.href(`/members/${member.member_id}?tab=Risk+profile`)}>Open profile<ArrowUpRight size={13} /></Link></td></tr>)}</tbody></table></div> : <Empty title="No member results yet" description="Start a calculation or inspect model and input readiness." />}
      </Panel>
    </> : null}
  </div>;
}
...
export function RiskRecapture({ user }: { user: User }) {
  ...
  useEffect(() => setInventoryPage(1), [tab, context.configId]);
