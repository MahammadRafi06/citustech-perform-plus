  ...
  return <div className="risk-workspace">
    ...
    <div className="risk-lab-layout">
      ...
      <div className="risk-workspace"><Panel title="Import score records" subtitle="Missing or invalid scores are listed for you to check."><div className="risk-form">
        ...
        {rows.length > 0 && <><span className="risk-field-label">{fileName} · {rows.length} records</span><div className="table-scroll"><table className="risk-table"><thead><tr><th>Member</th><th>Member rating group</th><th>Covered months</th><th>Score before adjustments</th></tr></thead><tbody>{rows.slice(0,8).map((row, i) => <tr key={i}><td>{String(row.member_id || "Missing")}</td><td>{String(row.rate_cell || "Missing")}</td><td>{String(row.coverage_months ?? "Missing")}</td><td>{typeof row.raw_score === "number" ? formatRiskScore(row.raw_score) : "Missing"}</td></tr>)}</tbody></table></div></>}
