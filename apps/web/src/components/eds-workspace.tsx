...
import { DEFAULT_EDS_FILTERS, EDS_CONTRACTS, EDS_MONTHS, EDS_NETWORKS, EDS_RECORDS, filterEdsRecords, memberScores, modelVersions, type EdsFilters, type EdsRecord } from '@/lib/eds-data';
import { buildEdsReport, defaultDataLabel, edsDisplayLabel, edsProcessingLabel, edsSupportLabel, EDS_REPORTS, EDS_SECTIONS, recordQualityIssue, type EdsChart, type EdsPoint, type EdsReport } from '@/lib/eds-reports';
import { edsRecordSubset, edsStageGaps } from '@/lib/eds-selection';
import { PaginatedTable } from './table-pagination';
import { Drawer } from './shared';
import { Button } from './ui/button';
import s from './eds-workspace.module.css';
const colors=['var(--sapphire)','var(--royal)','var(--green)','var(--yellow)','var(--regent)','var(--chart-6)'];
const count=(n:number)=>n.toLocaleString('en-US');
const axis={tick:{fill:'var(--regent)',fontSize:12},axisLine:false,tickLine:false};
const tooltip={border:'1px solid var(--line)',borderRadius:4,fontSize:13};
...
function Panel({title,children,tools}:{title:string;children:ReactNode;tools?:ReactNode}) {
 return <section className={`ct-card ${s.panel}`}><div className={s.panelHead}><h2>{title}</h2>{tools&&<div className={s.tools}>{tools}</div>}</div>{children}</section>;
}
function ReportChart({chart,onSelect,selection}:{chart:EdsChart;onSelect:(name:string,ids:string[])=>void;selection:string}) {
 ...
 const legend=<div className={s.legend}>{series.map((label,i)=><span key={label}><i style={{background:i===1&&(kind==='area'||chart.secondaryPercent)?colors[2]:colors[i]}}/>{label}</span>)}</div>;
 ...
 if(!points.length||points.every(p=>!p.value&&!p.secondary)) content=<div className={s.emptyChart}>No matching findings in this selection.</div>;
  ...
  const previous=points[i-1],lost=previous?edsStageGaps(previous.ids,p.ids):[];
  return <div key={p.name}><button className={s.funnelRow} aria-pressed={selection===p.name} onClick={()=>choose(p)}><span>{p.name}</span><div className={s.funnelTrack}><i style={{width:`${points[0].value?p.value/points[0].value*100:0}%`,background:colors[i%colors.length]}}/></div><strong>{count(p.value)}</strong><small>{points[0].value?(p.value/points[0].value*100).toFixed(1):'0'}%</small></button>{i>0&&<button className={s.gap} disabled={!lost.length} onClick={()=>onSelect(`${previous.name} → ${p.name}`,lost)}>{count(lost.length)} {series[0]==='Diagnosis Codes'?'diagnosis gaps':'encounter gaps'} <ChevronRight size={12}/></button>}</div>;
 })}</div>;
  ...
  content=<>{kind==='scatter'?<div className={s.legend}>{series[1]}</div>:legend}<div className={s.plot}><ResponsiveContainer width="100%" height={270} initialDimension={{width:700,height:270}}>
   {kind==='area'?<AreaChart data={points} margin={{top:12,right:22,left:10,bottom:2}} onClick={click}><CartesianGrid vertical={false} stroke="var(--line)"/><XAxis dataKey="name" {...axis}/><YAxis {...axis} tickFormatter={v=>display(v,unit)} width={58} domain={unit==='percent'?[0,100]:[0,'auto']}/><Tooltip contentStyle={tooltip} formatter={value=>display(Number(value),unit)}/><Area name={series[0]} type="monotone" dataKey="value" stroke={colors[0]} strokeWidth={2.4} fill={colors[1]} fillOpacity={.13} dot={{r:3,fill:colors[0]}} activeDot={{r:5}} isAnimationActive={false}/>{series[1]&&<Area name={series[1]} type="monotone" dataKey="secondary" stroke={colors[2]} fill="none" strokeWidth={2} isAnimationActive={false}/>}</AreaChart>
   :kind==='scatter'?<ScatterChart margin={{top:10,right:28,left:12,bottom:20}}><CartesianGrid stroke="var(--line)"/><XAxis {...axis} type="number" dataKey="value" name={series[0]} tickFormatter={v=>display(v,unit)} domain={[0,Math.max(...points.map(p=>p.value))*1.15+1]} label={{value:series[0],position:'insideBottom',offset:-12,fontSize:12}}/><YAxis {...axis} type="number" dataKey="secondary" name={series[1]} unit="%" width={48} domain={[0,110]} ticks={[0,25,50,75,100]}/><ZAxis dataKey="volume" range={[150,800]}/><Tooltip contentStyle={tooltip} cursor={{strokeDasharray:'3 3'}}/><Scatter name="HCC" data={points} onClick={p=>choose(p)} isAnimationActive={false}>{points.map((p,i)=><Cell key={p.name} fill={colors[i%colors.length]} fillOpacity={.8} stroke="white" strokeWidth={2}/>)}</Scatter></ScatterChart>
   :<ComposedChart data={points} margin={{top:8,right:kind==='pareto'?20:12,left:4,bottom:38}} onClick={click}><CartesianGrid vertical={false} stroke="var(--line)"/><XAxis dataKey="name" {...axis} interval={0} tick={<WrappedTick/>} height={52}/><YAxis {...axis} tickFormatter={v=>display(v,unit)} width={65}/>{chart.secondaryPercent&&<YAxis yAxisId="right" orientation="right" {...axis} domain={[0,100]} tickFormatter={v=>`${v}%`}/>}<Tooltip contentStyle={tooltip} formatter={(value,name)=>[display(Number(value),name===series[1]&&chart.secondaryPercent?'percent':unit),name]}/>{kind==='waterfall'&&<Bar dataKey="offset" stackId="waterfall" fill="transparent" isAnimationActive={false} legendType="none" tooltipType="none"/>}<Bar dataKey="value" name={series[0]} fill={colors[0]} stackId={kind==='waterfall'?'waterfall':undefined} maxBarSize={60} radius={[3,3,0,0]} isAnimationActive={false}>{points.map((p,i)=><Cell key={p.name} fill={kind==='waterfall'||series.length===1?colors[i%colors.length]:colors[0]}/>)}</Bar>{series[1]&&(chart.secondaryPercent?<Line dataKey="secondary" name={series[1]} yAxisId="right" stroke={colors[2]} strokeWidth={2.5} isAnimationActive={false}/>:<Bar dataKey="secondary" name={series[1]} fill={colors[1]} maxBarSize={60} radius={[3,3,0,0]} isAnimationActive={false}/>)}{series[2]&&<Line dataKey="tertiary" name={series[2]} stroke={colors[2]} strokeWidth={2} isAnimationActive={false}/>}</ComposedChart>}
 ...
 return <Panel title={chart.title}>{content}</Panel>;
}
function WrappedTick({x=0,y=0,payload}:{x?:number;y?:number;payload?:{value:string}}) {
 ...
 return <text x={x} y={y} textAnchor="middle" fill="var(--regent)" fontSize={11}>{lines.slice(0,3).map((line,i)=><tspan x={x} dy={i?14:17} key={i}>{line}</tspan>)}</text>;
}
export function EdsWorkspace({user}:{user:User}) {
 ...
 return <div className={s.workspace}><h1 className="sr-only">EDS Analytics</h1>
  ...
  </div><div className={s.filterTools}><button className={s.textButton} onClick={()=>setMore(!more)} aria-expanded={more} aria-controls="eds-more-filters"><SlidersHorizontal size={14}/>Encounter Filters<ChevronDown size={14}/></button><span>{filters.program==='Part C'?'CMS-HCC V28':'RxHCC'} · Payment Year {filters.paymentYear}</span>{hasFilters&&<button className={s.textButton} onClick={()=>update(Object.fromEntries(Object.keys(filters).map(k=>[`eds_${k}`,''])))}><X size={13}/>Reset Filters</button>}</div>
  {more&&<div id="eds-more-filters" className={s.moreFilters}><Field label="PBP" value={filters.pbp} onChange={v=>change('pbp',v)} options={['001','002','003']} all="All PBPs"/><Field label="837 Type" value={filters.type} onChange={v=>change('type',v)} options={[['837-P','Professional (837-P)'],['837-I','Institutional (837-I)'],['837-P · DME','DME (837-P)']]} all="All Types"/><Field label="Record Type" value={filters.recordType} onChange={v=>change('recordType',v)} options={[['EDR','Encounter Data Record (EDR)'],['CRR','Chart Review Record (CRR)']]} all="EDR & CRR"/><Field label="Source System" value={filters.source} onChange={v=>change('source',v)} options={['Claims Feed','Clinical EHR','Chart Review'].map(value=>[value,edsDisplayLabel(value)])} all="All Sources"/><Field label="Service Month" value={filters.month} onChange={v=>change('month',v)} options={EDS_MONTHS.map((m,i)=>[String(i+1),m])} all="All Months"/><Field label="Submitter" value={filters.submitter} onChange={v=>change('submitter',v)} options={['Perform+ EDI','Network Exchange']} all="All Submitters"/><Field label="Environment" value={filters.environment} onChange={v=>change('environment',v)} options={['Production','Test']}/></div>}</section>
  ...
  <section id="eds-report-panel" role="tabpanel" aria-labelledby={`eds-tab-${EDS_SECTIONS.indexOf(active.section)}`}>
   <div className={s.reportBar}><div><h2>{report.title}</h2></div><label className={s.reportPicker}><span>Report</span><select aria-label="EDS Report" value={active.id} onChange={e=>update({report:e.target.value})}>{EDS_REPORTS.filter(r=>r.section===active.section).map(r=><option key={r.id} value={r.id}>{r.id==='prevalence'&&filters.program==='Part D'?'RxHCC Prevalence Trends':r.title}</option>)}</select></label></div>
...
function ReportBody({id,report,rows,filters,user}:{id:string;report:EdsReport;rows:EdsRecord[];filters:EdsFilters;user:User}) {
 ...
 const selectedRows=useMemo(()=>selected?edsRecordSubset(rows,selected.ids):report.records,[rows,selected,report.records]);
 ...
 const recordScores=useMemo(()=>new Map(memberScores(rows,filters.program,Number(filters.paymentYear)).map(m=>[m.memberId,m])),[rows,filters.program,filters.paymentYear]);
 const sortValue=(r:EdsRecord,column:number)=>{
  const score=recordScores.get(r.memberId);
  if(column===0)return r.memberName;
  if(column===1)return report.table==='quality'?recordQualityIssue(r,id):report.table==='lifecycle'?`${r.recordType} · ${edsDisplayLabel(r.action)}`:report.table==='score'?score?.local:r.icd;
  if(column===2)return report.table==='score'?score?.cms:report.table==='diagnosis'?(filters.program==='Part C'?(r.hcc?`HCC-${r.hcc}`:'Not Mapped'):(r.rxhcc?`RxHCC-${r.rxhcc}`:'Not Mapped')):r.network;
  if(column===3)return report.table==='score'?score?.gap:report.table==='lifecycle'?r.parentIcn||'Unlinked':r.serviceDate;
  if(column===4)return report.table==='diagnosis'?edsSupportLabel(r):report.table==='score'?(filters.program==='Part C'?'CMS-HCC V28':'RxHCC'):edsProcessingLabel(r.stage);
  return report.table==='diagnosis'?edsDisplayLabel(r.final):report.table==='lifecycle'?edsDisplayLabel(r.lifecycleIssue)||(r.exception?'Member-Switch Exception':r.unlinked?'No Encounter Link':'Linked to Encounter'):report.table==='score'?(score?.gap?'Risk Score Difference':'Scores Match'):edsDisplayLabel(r.issue);
 };
 return <div className={s.reportBody}><div className="ct-metric-grid">{report.metrics.map(m=><div className="ct-metric" key={m.label}><div className="ct-metric-label">{m.label}</div><div className="ct-metric-value">{m.value}</div><div className="ct-metric-note">{m.note}</div></div>)}</div>
  ...
  <Panel title={report.tableTitle} tools={selected?<button className={s.selection} onClick={()=>setSelected(null)}>{selected.name}<X size={13}/></button>:<span className={s.recordCount}>{count(visible.length)} Records</span>}>
   {!visible.length?<div className={s.noRecords}><h3>No Matching Records</h3><p>{rows.length?'No records meet this report’s criteria for the selected population.':'Change the filters to include another population.'}</p>{selected&&<Button variant="outline" onClick={()=>setSelected(null)}>Clear Chart Selection</Button>}</div>:<PaginatedTable sortValue={sortValue} rows={visible} scope={`${id}:${selected?.name||''}`} label={report.tableTitle} headers={<><th scope="col">{report.table==='score'?'Member':'Encounter / Member'}</th><th scope="col">{report.table==='quality'?(id==='defaults'?'Default Data Reason':'Data Issue'):report.table==='lifecycle'?'Record Type / Action':report.table==='score'?'Calculated Score':'Diagnosis Code'}</th><th scope="col">{report.table==='score'?'Reconciled Score':report.table==='diagnosis'?'HCC / RxHCC':'Health Network'}</th><th scope="col">{report.table==='score'?'Score Gap':report.table==='lifecycle'?'Original Encounter ICN':'Service Date'}</th><th scope="col">{report.table==='diagnosis'?'Clinical Documentation':report.table==='score'?'Model':'Encounter Processing Status'}</th><th scope="col">{report.table==='diagnosis'?'MAO-004 RA Eligibility':report.table==='lifecycle'?'Encounter Linkage Status':'Status / Issue'}</th><th scope="col"><span className="sr-only">Details</span></th></>}>
    {r=>{const score=recordScores.get(r.memberId);return <tr key={r.id}><td><button className={s.recordLink} onClick={()=>setRecord(r)}>{r.memberName}</button>{report.table!=='score'&&<small className={s.cellNote}>{r.id}</small>}</td><td>{report.table==='quality'?recordQualityIssue(r,id):report.table==='lifecycle'?`${r.recordType} · ${edsDisplayLabel(r.action)}`:report.table==='score'?score?.local.toFixed(3):<>{r.icd}<small className={s.cellNote}>{r.condition}</small></>}</td><td>{report.table==='score'?score?.cms.toFixed(3):report.table==='diagnosis'?(filters.program==='Part C'?(r.hcc?`HCC-${r.hcc}`:'Not Mapped'):(r.rxhcc?`RxHCC-${r.rxhcc}`:'Not Mapped')):r.network}</td><td>{report.table==='score'?score?.gap.toFixed(3):report.table==='lifecycle'?r.parentIcn||'Unlinked':r.serviceDate}</td><td>{report.table==='diagnosis'?edsSupportLabel(r):report.table==='score'?filters.program==='Part C'?'CMS-HCC V28':'RxHCC':edsProcessingLabel(r.stage)}</td><td>{report.table==='diagnosis'?edsDisplayLabel(r.final):report.table==='lifecycle'?edsDisplayLabel(r.lifecycleIssue)||(r.exception?'Member-Switch Exception':r.unlinked?'No Encounter Link':'Linked to Encounter'):report.table==='score'?(score?.gap?'Risk Score Difference':'Scores Match'):edsDisplayLabel(r.issue)}</td><td><button className={s.openRecord} onClick={()=>setRecord(r)} aria-label={`Inspect ${r.id}`}><ArrowUpRight size={17}/></button></td></tr>;}}
   </PaginatedTable>}
...
function RecordDrawer({record:r,filters,onClose,canOpenMember}:{record:EdsRecord|null;filters:EdsFilters;onClose:()=>void;canOpenMember:boolean}) {
 ...
 return <Drawer open={!!r} onOpenChange={open=>{if(!open)onClose();}} title="Encounter Details" description={r?`${r.memberName} · ${r.id}`:''} className={s.drawer}>{r&&<div className={s.lineage}>
  ...
  <section><h3>Source Encounter</h3><dl className={s.facts}>{[['Data Source',r.profile?'Member 360 · Risk Adjustment':edsDisplayLabel(r.source)],['Service Date',r.serviceDate],['Ready for Submission',r.sourceReady],['Record Type',`${r.recordType==='CRR'?'Chart Review Record':'Encounter Data Record'} · ${r.type}`],['Diagnosis Code',`${r.icd} · ${r.condition}`],['Clinical Documentation',edsSupportLabel(r)]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl></section>
  <section><h3>EDS Processing History</h3><ol className={s.timeline}>{[
   ['Source Encounter','Ready for Submission',r.sourceReady],['EDS Submission (837)',r.stage>0?'Submitted to EDS':'Not Submitted',r.submittedAt],['EDFES File Validation (999)',r.stage>=2?'Passed':r.stage===1?'Not Accepted':'Not Reached',r.stage>=2?r.ackAt:null],['EDFES Acceptance (277CA)',r.stage>=3?'Accepted · ICN Assigned':'Not Reached',r.stage>=3?r.ackAt:null],['EDPS Acceptance (MAO-002)',r.stage===4?'Encounter Accepted':r.stage===3?'Not Yet Accepted':'Not Reached',r.acceptedAt],['RA Eligibility (MAO-004)',edsDisplayLabel(r.final),r.finalAt],
  ...
  <section><h3>Risk Adjustment Eligibility</h3><dl className={s.facts}><div><dt>Preliminary RA Eligibility (MAO-002)</dt><dd>{r.preliminary==='Pending'?'Preliminary Eligibility Pending':edsDisplayLabel(r.preliminary)}</dd></div><div><dt>Final RA Eligibility (MAO-004)</dt><dd>{edsDisplayLabel(r.final)}</dd></div><div><dt>Eligibility Reason</dt><dd>{edsDisplayLabel(r.finalReason)}</dd></div><div><dt>Clinical Documentation</dt><dd>{edsSupportLabel(r)}</dd></div></dl></section>
  <section><h3>Encounter References</h3><dl className={s.facts}>{[['Source Encounter ID',r.id],['Submission File / Claim ID',`${r.file} / ${r.id}`],['Encounter ICN',r.icn||'Not Assigned'],['Original Encounter ICN',r.parentIcn||'No Encounter Link'],['Chart Review Action',r.recordType==='CRR'?edsDisplayLabel(r.action):'Not Applicable'],['Submission Issue',r.edit?`${edsDisplayLabel(r.edit)}${/^\d+$/.test(r.edit)?` · ${r.edit}`:''}`:'No Submission Issue'],['Encounter Status',edsDisplayLabel(r.lifecycleIssue||r.issue)],...(r.defaultReason?[['Default Data Reason',`${defaultDataLabel(r.defaultReason)} · ${r.defaultReason}`]]:[])].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl>{r.parentIcn&&<div className={s.parentChain}><span>Original Encounter<br/>{r.parentIcn}</span><ChevronRight size={16}/><span>{edsDisplayLabel(r.action)}<br/>{r.icn||'Pending ICN'}</span></div>}</section>
  <section><h3>Risk Model & Reporting Period</h3><dl className={s.facts}>{[['Service Year',versions.serviceYear],['Payment Year',versions.paymentYear],['Model',versions.model],['Diagnosis Mapping Version',versions.mapping],['Eligibility Rules Version',versions.filter],['Risk Model Coefficients',versions.coefficient]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><a className={s.referenceLink} href="https://www.cms.gov/newsroom/press-releases/cms-finalizes-2027-medicare-advantage-part-d-payment-policies-strengthen-accountability-long-term" target="_blank" rel="noreferrer">CMS 2027 Policy Reference <ArrowUpRight size={14}/></a></section>
 </div>}</Drawer>;
}
