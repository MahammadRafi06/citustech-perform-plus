...
export function RiskFinancial({ user }: { user: User }) {
  ...
  return <div className="risk-workspace"><div className="risk-tabs"><button aria-selected={tab === "calculate"} onClick={() => setTab("calculate")}>Calculate sensitivity</button><button aria-selected={tab === "saved"} onClick={() => setTab("saved")}>Retained sensitivities</button></div>
    {tab === "saved" ? <Panel title="Retained financial assumptions and outputs">{saved.isPending ? <div className="risk-loading">Loading retained sensitivities…</div> : saved.error ? <Empty title="History unavailable" description={saved.error.message} /> : saved.data?.items.length ? <div className="table-scroll"><table className="risk-table"><thead><tr><th>Saved sensitivity</th><th>Program</th><th>State</th><th>Estimated difference</th><th>Inspect</th></tr></thead><tbody>{saved.data.items.map((item) => <tr key={item.id}><td><strong>{item.name || item.id}</strong><small>{new Date(item.created_at).toLocaleString()}</small></td><td>{item.program}</td><td>{label(item.status)}</td><td>{money(item.estimate?.difference)}</td><td><Button variant="ghost" size="sm" onClick={() => { setResult(item); setTab("result"); }}>Open retained result</Button></td></tr>)}</tbody></table></div> : <Empty title="No saved financial sensitivities" />}</Panel> : tab !== "result" ? !program ? <Empty title="No financial method for this program" description="Select a configured MA, ACA or external Medicaid context. Part D scores retain their own program basis." /> : <>
    ...
    {result && tab !== "saved" && <Panel title={result.estimate?.label || "Retained financial sensitivity"} subtitle={`${result.id} · ${result.program}`} action={context.permissions.includes("export") && <Button variant="outline" size="sm" onClick={() => downloadRiskJson(result, `${result.id}.json`)}>Export retained result</Button>}>
      ...
      {!!result.components.length && <div className="table-scroll"><table className="risk-table"><thead><tr><th>Component</th><th>Baseline</th><th>Scenario</th><th>Difference</th></tr></thead><tbody>{result.components.map((item) => <tr key={item.key}><td>{item.label}</td><td>{money(item.baseline)}</td><td>{money(item.scenario)}</td><td>{money(item.difference)}</td></tr>)}</tbody></table></div>}
      {!!result.sensitivities.length && <div className="table-scroll"><table className="risk-table"><thead><tr><th>Assumption sensitivity</th><th>Baseline estimate</th><th>Scenario estimate</th><th>Difference</th></tr></thead><tbody>{result.sensitivities.map((item, index) => <tr key={`${item.label}-${index}`}><td>{label(item.label)}</td><td>{money(item.baseline)}</td><td>{money(item.scenario)}</td><td>{money(item.difference)}</td></tr>)}</tbody></table></div>}
