...
function Paged<T>({rows,headers,render,label,scope,tableClassName=''}:{rows:T[];headers:ReactNode;render:(row:T,index:number)=>ReactNode;label:string;scope?:string;tableClassName?:string}) {
 ...
 return <><div className={styles.tableWrap}><SortableTable key={scope} className={`${styles.table} ${tableClassName}`} aria-label={label} pageStart={current*size} pageSize={size} onSortChange={()=>setPage(0)}><thead><tr>{headers}</tr></thead><tbody>{rows.map((row,i)=>render(row,i))}</tbody></SortableTable>{!rows.length&&<EmptyScope/>}</div><TablePagination totalRows={rows.length} pageIndex={current} pageSize={size} onPageChange={setPage} onPageSizeChange={v=>{setSize(v);setPage(0);}} label={label}/></>;
}
...
function Trend({report}:{report:AnalysisReport}) {
 ...
 // Display submitted and accepted scores only through August; retain the source report and score tiles.
 const trend=report.trend.map(point=>['Sep','Oct','Nov','Dec'].includes(point.month)?{...point,submitted:null,accepted:null}:point);
 return <><div className={styles.legend}>{['captured_baseline','potential','submitted','accepted'].map(key=>scoreBasisLabel(key,report.config.program)).map((n,i)=><span key={n}><i style={{background:COLORS[i]}}/>{n}</span>)}</div>
  <Chart label="Year-to-date population RAF trend" height={280}>
   <ComposedChart data={trend} margin={{left:0,right:14,top:12,bottom:0}}>
...
export function AnalyticsWorkspace({user,route}:{user:User;route:string}) {
 ...
 const title=route==='overview'?'Dashboard':route==='reports'?'Reports':route==='suspects'?'Suspect Analytics':'Risk Analytics';
...
function DiscoveryCaseTable({report,selected,toggle,focus}:{report:AnalysisReport;selected:string[];toggle:(id:string)=>void;focus:(c:SuspectCase)=>void}) {
 return <Paged rows={report.cases} scope={report.filter_hash} label="Clinical context conditions list" tableClassName={styles.registryTable}
  ...
  render={c=><tr key={c.id}>
   ...
   <td data-sort-value={c.member_name||c.member_id}><CaseMemberName finding={c}/></td>
   ...
   <td className={styles.number} data-sort-value={c.profile_reference?c.profile_reference.delta:c.delta}><CaseImpact finding={c}/></td>
  </tr>}/>;
}
