import { edsRecordSubset } from './eds-selection';
...
function groups(rows:EdsRecord[],key:(r:EdsRecord)=>string,limit=10):EdsPoint[] {
 const groups=new Map<string,EdsRecord[]>(); for(const r of rows){const k=key(r);if(k){const group=groups.get(k);if(group)group.push(r);else groups.set(k,[r]);}}
...
export function buildEdsReport(id:string,rows:EdsRecord[],filters:EdsFilters,providerScope=''):EdsReport {
 ...
 if(id==='acceptance') {
  ...
  const types=groups(rows,r=>edsDisplayLabel(r.serviceCategory)).map(p=>{const r=edsRecordSubset(rows,p.ids);return {...p,value:count(r,x=>x.stage>0),secondary:count(r,x=>x.stage===4),tertiary:r.length};});
  const nets=groups(rows,r=>r.network).map(p=>({...p,denominator:p.value,value:count(edsRecordSubset(rows,p.ids),r=>r.stage>0)/p.value*100}));
  ...
  const variance=groups(rows,modelCode,5).map(p=>{const m=memberScores(edsRecordSubset(rows,p.ids),filters.program,Number(filters.paymentYear));return {...p,value:average(m.map(x=>x.local)),secondary:average(m.map(x=>x.cms))};});
  ...
  const rates=groups(mapped,r=>edsDisplayLabel(r.serviceCategory)).map(p=>({...p,denominator:p.value,value:count(edsRecordSubset(mapped,p.ids),r=>r.serviceEligible)/p.value*100}));
  ...
  const risks=groups(unsupported,modelCode,5).map(p=>({...p,volume:p.value,value:memberScores(edsRecordSubset(unsupported,p.ids),filters.program,Number(filters.paymentYear)).reduce((sum,m)=>sum+m.audit*paymentFactor,0),secondary:p.value?count(edsRecordSubset(unsupported,p.ids),r=>r.final==='Allowed')/p.value*100:0,detail:p.name}));
  ...
  result.charts=[chart('Documentation Risk by Condition','scatter',risks,['Estimated Payment Risk','MAO-004 Eligible Codes (%)'],'Each bubble groups diagnosis codes with insufficient documentation. The vertical axis shows the share that still has MAO-004 eligibility; the horizontal axis shows estimated payment exposure from paired member-level scenarios. Bubble size is the diagnosis count. Annual assumptions are $12,000 per Part C score point or $1,800 per Part D subsidy factor.',{unit:'currency'}),chart('Documentation Support by Source','bar',groups(reviewed,r=>edsDisplayLabel(r.source)).map(p=>({...p,value:count(edsRecordSubset(reviewed,p.ids),r=>r.supported),secondary:count(edsRecordSubset(reviewed,p.ids),r=>!r.supported)})),['Supported','Insufficient Clinical Evidence'],'Reviewed diagnosis codes grouped by source and documentation support.')];result.records=unsupported;result.table='diagnosis';result.tableTitle='Documentation Review Findings';
  ...
  const points=groups(eligible,modelCode,5).map(p=>{const memberCount=new Set(edsRecordSubset(eligible,p.ids).map(r=>r.memberId)).size;return {...p,value:pop?memberCount/pop*100:0,denominator:pop};});
